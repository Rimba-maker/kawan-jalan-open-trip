...
components:
  ...
  badge-negative:
    ...
    padding: "{spacing.xs} {spacing.md}"
  badge-vibe-tag:
    description: "Vibe Matching tag pill — background swaps per vibe (chill/adventure/content/party/family), text always {colors.canvas} white."
    backgroundColor: "{colors.vibe-chill} | {colors.vibe-adventure} | {colors.vibe-content} | {colors.vibe-party} | {colors.vibe-family}"
    textColor: "{colors.canvas}"
    typography: "{typography.body-sm-strong}"
    rounded: "{rounded.pill}"
    padding: "{spacing.xs} {spacing.md}"
  card-trip-schedule:
    description: "Jadwal Trip card — re-uses currency-converter-card chrome; hosts destination photo, date, price, and a slot-progress-bar filled with {colors.primary}."
    backgroundColor: "{colors.canvas}"
    textColor: "{colors.ink}"
    borderColor: "{colors.ink}"
    typography: "{typography.body-md}"
    ...
    progressTrack: "{colors.canvas-soft}"
  footer:
    backgroundColor: "{colors.ink}"
...
#### Image Behavior — tailored for Kawan Jalan
Wise's native brand keeps photography sparse in favor of illustrative SVGs — **overridden for this project**. Kawan Jalan is photography-forward: every section (hero, Jadwal Trip cards, Destinasi grid, Vibe Matching, Cerita Dari Trip) anchors on a real destination or group photo. Treat photos as full-bleed media inside `{rounded.xl}` card corners, same radius as every other card, so photography and UI chrome share one shape language instead of competing.
